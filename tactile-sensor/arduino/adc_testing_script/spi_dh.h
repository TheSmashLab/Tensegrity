...
class SPIClass {
  public:
	...
	// Transfer functions
	uint16_t transfer(byte _pin, uint8_t _data, SPITransferMode _mode = SPI_LAST);
	...
	uint16_t transfer(uint8_t _data, SPITransferMode _mode = SPI_LAST) { return transfer(BOARD_SPI_DEFAULT_SS, _data, _mode); }
