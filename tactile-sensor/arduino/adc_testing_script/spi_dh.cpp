 ...
 * 
 * Altered by David Hill for a faster, streamlined communication. Do not use
 * this file unless for tactile sensing.
 */
...
void SPIClass::setDataMode(uint8_t _pin, uint8_t _mode) {
	...
	SPI_ConfigureNPCS(spi, ch, mode[ch] | SPI_CSR_SCBR(divider[ch]) | SPI_CSR_BITS_16_BIT | SPI_CSR_DLYBCT(0));
}
...
uint16_t SPIClass::transfer(byte _pin, uint8_t _data, SPITransferMode _mode) {
	uint32_t ch = BOARD_PIN_TO_SPI_CHANNEL(_pin);

	uint32_t d = _data | SPI_PCS(1) | SPI_TDR_LASTXFER;
