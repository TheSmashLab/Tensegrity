...
class tactile_visualization():
   ...
   def __init__(self):
      ...
      self.cyl.scale.y = 17
      self.cyl.scale.z = 22
      self.cyl.pose.position.x = -11.0
      self.cyl.pose.position.y = 0.0
      self.cyl.pose.position.z = 0.0

      quat = tft.quaternion_from_euler(0,pi/2,0)
      ...
      # create array of thetas to facilitate rotation for each arrow
      # self.theta = [pi/3,pi,5*pi/3,5*pi/3,pi,pi/3,-pi/3,-pi,-5*pi/3,-5*pi/3,pi,-pi/3]
      ...
      for i in range(0,12):
         ...
         self.arrow.scale.y = 5.0
         self.arrow.scale.z = 1.0
         self.arrow.pose.position.x = 100.0*self.data_in.position[i].x
         self.arrow.pose.position.y = 100.0*self.data_in.position[i].y 
         self.arrow.pose.position.z = 100.0*self.data_in.position[i].z
         ...
         quat_arrow = tft.quaternion_from_euler(self.theta[i],pi/2,0)
   ...
   def run(self):
      while not rospy.is_shutdown():
         ...
         for i in range(0,12):
            ...
            #self.arrow_array[i].pose.position.y = 100.0*self.data_in.position[i].y + self.arrow_array[i].scale.x*sin(self.theta[i])
            #self.arrow_array[i].pose.position.z = 100.0*self.data_in.position[i].z + self.arrow_array[i].scale.x*cos(self.theta[i])
