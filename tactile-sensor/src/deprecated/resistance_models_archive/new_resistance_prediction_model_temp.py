...
from tactile_sensor.msg import ADC as analog
...
class grid_visualization():
    ...
    def __init__(self):
        ...
        # initialize publisher for analog voltage readings
        self.pub_ADC = rospy.Publisher('/ADC', analog, queue_size = 1)

        # initialize a publisher for the predicted forces.
        self.pub_forces = rospy.Publisher('/calibrated_forces', res, queue_size = 1)
    ...
    def run(self):
        ...
        R = 330.0 # constant resistor value
        vout_mat = np.empty((self.m, self.n, self.n))
        Ic_temp = np.empty((self.m,self.n))
        temp = np.array(np.zeros((self.m*self.n,1)))


        # create offset values for zeroing initial resistances
        for i in range(0, 1000):
            voltage = self.ADC_reading #*5.0/ADC
            ...
            R_predicted[R_predicted > 4000.0] = 4000.0

            temp = R_predicted + temp
        ...
        while not rospy.is_shutdown():
            ...
            R_predicted = R_predicted + (4000.0-offset)

            # pdb.set_trace()

            R_reshaped = np.reshape(R_predicted,(self.m,self.n))
            # print str(R_predicted) + '\n'

            force_calibrated = -0.0015*R_predicted + 6.0
            ...
            self.analog.ADC = np.reshape(self.ADC_reading,(self.m*self.n,1))
            self.pub_ADC.publish(self.analog)
            

            # publish calibrated forces
            self.pub_forces.publish(force_calibrated)

            print(np.sum(force_calibrated))
